...
class StagiaireActionController extends Controller
{
    ...
    public function cvUpload(Request $request)
    {
        if ($request->hasFile('cv')) {
            ...
            $path = Storage::disk('resumes')->putFileAs('/', $file, $user->matricule . $user->nom . $user->prenom . '.' . $file->extension());
            $user->cv = $path;
            $user->save();

            return redirect()->route('profile.invitation');
        } else return redirect()->back()->withErrors(['error' => 'Error :try again later']);
    }
